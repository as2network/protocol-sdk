// @file api 
// @summary as2network api endpoint
import fetch from 'cross-fetch';

export type AS2Network = [3, 'ropsten'] | [1, 'mainnet'];

export type RelaySpeed = 'fastest' | 'fast' | 'average' | 'safelow';

export type AS2NetworkApiOpts = {
  baseUrl: string,
  token?: string,
  apikey?: string,
  network: AS2Network;
...
export class AS2NetworkApi {
  private readonly opts: AS2NetworkApiOpts;
  private readonly headers: { [key: string]: string };

  private generateHeaders(opts: AS2NetworkApiOpts): { [key: string]: string } {
  ...
  private authenticationChecks(opts: AS2NetworkApiOpts): void {
  ...
  constructor(opts: AS2NetworkApiOpts) {
