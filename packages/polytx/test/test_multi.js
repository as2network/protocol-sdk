...
contract("MultiCaller", async () => {
    describe("executeWithFunctionName()", () => {
        it("should call function without arguments by name repeatedly", async () => {
            ...
            const numCalls = 100;
            await multiCaller.executeWithFunctionName(
                dummy.address,
                numCalls,
                "incrementValue()"
            );
            ...
            assert.equal(
                valueAfter,
                numCalls,
                "Expected value should be numCalls"
            );
        });
        ...
        it("should fail when calling non-existent function", async () => {
            ...
            await truffleAssert.reverts(
                multiCaller.executeWithFunctionName(
                    dummy.address,
                    1,
                    "nonExistantFunction()"
                ),
    ...
    describe("executeWithCalldata()", () => {
        it("should call function with arguments repeatedly", async () => {
            ...
            const numCalls = 100;
            const increment = 42;
            const calldata = dummy.contract.methods
                .incrementBy(increment)
                .encodeABI();

            await multiCaller.executeWithCalldata(
                dummy.address,
                numCalls,
                calldata
            );
