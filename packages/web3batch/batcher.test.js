...
async function deployContract(filename, name) {
    ...
    const contractBytecode =
        contractInfo.contracts[filename][name].evm.bytecode.object;
    const contractToDeploy = new web3.eth.Contract(contractABI);
    const deployedContract = await contractToDeploy
        .deploy({
...
test("send some transactions directly to the contract", async () => {
    ...
    tx = await contract.methods
        .batchSend(
            ...
            [0],
            [dummyContract.methods.foo().encodeABI()]
        )
    ...
    tx = await contract.methods
        .batchSend(
            ...
            [
                dummyContract.methods.foo().encodeABI(),
                dummyContract.methods.bar().encodeABI(),
            ]
        )
        .send({ from: web3.eth.defaultAccount });
...
test("batch some transactions using the batcher", async () => {
    ...
    expect(new BN(balanceAfter).sub(new BN(balanceBefore)).toNumber()).toEqual(
        ethToSend
    );
});
